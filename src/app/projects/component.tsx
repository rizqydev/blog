...
export default function ProjectClients() {
  ...
  return (
    <>
      ...
      <div className="flex flex-col flex-nowrap gap-2 md:flex-row md:flex-wrap md:justify-between md:gap-0">
        {cards.map((card, key) => (
          <div
            key={key}
          ...
          <>
            ...
            <div className="px-3 py-4 dark:bg-slate-800">
              ...
              <div className="flex flex-wrap gap-2 text-sm text-slate-800 dark:text-slate-100">
                <div className="font-light text-opacity-70">Built with:</div>
                {cardSelected?.builtWith.map((item, index) => (
                  <div
                    key={index}
                    className="rounded-md bg-slate-500 px-3 py-1 text-xs text-slate-50"
                  >
                    {item}
                  </div>
                ))}
